...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    $customer = UserController::getAssociatedCustomer($response['data']['id']);
    if ($response['status'] == "success") {
        $success_msg .= $response["message"];


        $_SESSION['sessCustomerID'] = $customer['id'];
        $_SESSION['username'] = $response['data']['username']; // Initializing Session
        $_SESSION['user_id'] = $response['data']['id']; // Initializing Session
